...
import math
import numpy as np
def mark_path_on_grid(path, occupancy_grid, occupancy_grid_size=80):

    # Scale factor to match path coordinates to grid indices
    scale_factor = 10
        
    for point in path:
        grid_x = int(point[0] * scale_factor)
        grid_y = int(point[1] * scale_factor)
        if 0 <= grid_x < occupancy_grid_size and 0 <= grid_y < occupancy_grid_size:
            occupancy_grid[grid_y][grid_x] = 1  # Mark the cell as occupied

# Save the grid to a CSV file
    filename = 'C:/Users/Sam/Documents/Webots_Sim/project files/controllers/Nav_control/mapped.csv'
    with open(filename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(occupancy_grid)
...
def addBufferToOccupancyGrid(occupancy_grid, buffer_width):
    ...
    occupancy_grid_buff_list = occupancy_grid_buff.tolist()

    # Define the filename
    filename = 'C:/Users/Sam/Documents/Webots_Sim/project files/controllers/Nav_control/bufferedzeros.csv'

    # Write the data to a CSV file
    with open(filename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(occupancy_grid_buff_list)
